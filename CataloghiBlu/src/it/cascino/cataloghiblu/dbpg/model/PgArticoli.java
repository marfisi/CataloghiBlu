package it.cascino.cataloghiblu.dbpg.model;
...
@NamedQueries({
		@NamedQuery(name = "PgArticoli.findAll", query = "SELECT a FROM Articoli a order by a.codice asc"),
		@NamedQuery(name = "PgArticoli.findById", query = "SELECT a FROM Articoli a WHERE a.id = :id"),
		@NamedQuery(name = "PgArticoli.findByCodiceArticolo", query = "SELECT a FROM Articoli a WHERE a.codice = :codiceArticolo")
})
public class PgArticoli implements Serializable{
	...
	public PgArticoli(Integer id, Integer articoloFornitore, String codice, 	String descrizioneAs400, String descrizione, String modello, String nome, Integer produttore, Integer tipo, Timestamp updtime){
		super();
		this.id = id;
		this.codice = codice;
